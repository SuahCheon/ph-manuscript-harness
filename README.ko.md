...
### 다른 런타임(Codex) — 같은 주제, 측정된 증거

SKILL.md *규율*은 오픈 Agent Skills 표준이라 Codex로 그대로 옮겨진다. *강제*는
그렇지 않다 — 그리고 그게 결함이 아니라 핵심이다. Codex의 hook은 현재 `Bash` 도구만
가로채고 `Write`/`Edit`는 못 잡아, 작성-시점 게이트를 박을 자리가 없다. 그래서 이식본
([`codex/`](codex/), [`codex/INSTALL.md`](codex/INSTALL.md) 참조)은 브레이크를 hook이
*실제로 닿을 수 있는* 다음 경계인 **git `pre-commit` 게이트**로 옮긴다. 라벨도 비대칭도
동일 — `[FABRICATED]` 인용은 **커밋을 중단**시키고, 보고 누락은 보고만 하고 막지 않는다.
즉 규율은 (같은 SKILL.md로) 옮겨졌고, 강제는 다른 런타임 이음매(쓰기 syscall → 커밋)에
다시 박혔다. 이것이 *규칙은 옮겨지고, 강제는 런타임에 박힌다* — 주장이 아니라 관측이다.
